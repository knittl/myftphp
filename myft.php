...
$user = &$_POST['user'];
	...
	if(isset($_POST['login'])) {
		...
		try {
			...
			echo '<div class="box login"><h1>ERROR</h1>';
			echo $e->getMessage();
			echo '</div>';
		}
		...
		$action = htmlspecialchars($_SERVER['REQUEST_URI']); #urlencode($_SERVER['REQUEST_URI'])
		if($_SERVER['REQUEST_URI'] != SELF) $action = dosid($action);
	?>
	<!-- -->
		<hr>
		<div class="box login">
