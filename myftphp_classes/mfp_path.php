...
class mfp_path {
	...
	// also quite moo'ish
	// could replace some of the following wrapper functions
	// !!! secure enough???
	...
	private function performCheck() {
		...
		if(!(strpos(realpath($this->path), REALHOME) === 0)) throw new Exception(sprintf($GLOBALS['l']['err']['forbidden'], $this->path));
	}
